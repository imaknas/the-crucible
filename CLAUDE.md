...
- **Compaction policy** (`app/compaction/policy.py`): `ThresholdPolicy` (default: the model's registry `limit`, i.e. the original behaviour), `NeverCompact`, and pluggable thresholds (`fixed_tokens`, `fraction_of_limit`). Inject per run with `config["configurable"]["compaction_policy"]`; the graph passes it a `ContextSnapshot` (token/message counts) and a `ModelBudget` (model facts). Re-summarizing waits for new material: due when summary + newer messages pass the threshold, but never before `min_new_fraction` (0.5) of the threshold is new — a summary larger than the threshold used to re-trigger on every eligible turn. `config["configurable"]["summarizer_model"]` overrides which model writes the summary (default: first registered of `SUMMARIZER_MODELS`).
...
| A detail-retention experiment | Facts/scenarios in `app/compaction/probe.py` (`build_dense_scenario` for real runs: specifics in every turn, distractor and update variants with `stale` values; `build_scenario` is pilot 1's generic filler). `services/compaction_eval.run_live_grid` replays the conversation turn by turn through the graph's own summarize step (several summaries, shared by all answering models); `run_grid` seeds it whole. The pseudo-model `oracle` (`EvalModelFactory`) answers iff the value is still in context, so it measures what the summaries removed, independent of a real model's attention. CLI: `crucible compaction-eval`; lab notes in `backend/experiments/NOTES.md`. |
